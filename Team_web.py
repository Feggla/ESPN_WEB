...
class Teams:
    def __init__(self, mw, player, place, league, scoreboard, score):
        ...
        if place == "away":
            self.stats_away()
        self.stat_list = [self.name, self.points, self.blocks, self.steals, self.assists, self.rebounds, self.turnovers, self.fg, self.ft, self.threes, self.three_percent, self.td]
